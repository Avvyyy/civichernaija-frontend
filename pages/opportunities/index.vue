  ...
  <div class="opportunities-page container">
    ...
    <div v-else class="opps-grid">
      <div class="glass-card opp-card" v-for="opp in opportunities" :key="opp._id">
        <div class="badge-type">{{ opp.type }}</div>
        <h3>{{ opp.title }}</h3>
...
<style scoped>
.opportunities-page { padding: 2rem 1rem; }
...
.opp-card { display: flex; flex-direction: column; position: relative; }
.badge-type { position: absolute; top: 1.5rem; right: 1.5rem; background: var(--secondary); color: #000; font-size: 0.75rem; font-weight: 700; padding: 0.25rem 0.75rem; border-radius: 999px; }
.location { font-size: 0.9rem; color: var(--text-muted); margin-bottom: 1rem; }
.desc { flex-grow: 1; color: var(--text-main); line-height: 1.5; margin-bottom: 1.5rem; }
.mt-4 { margin-top: auto; }
.inline-block { display: inline-block; text-align: center; text-decoration: none; }
.empty-state { text-align: center; max-width: 680px; margin: 1rem auto 0; }
...
.skeleton-chip { width: 90px; height: 24px; margin-left: auto; margin-bottom: 1rem; border-radius: 999px; }
.skeleton-title { width: 65%; height: 1.2rem; margin-bottom: 0.75rem; }
.skeleton-line { width: 100%; height: 0.9rem; margin-bottom: 0.6rem; }
.skeleton-line.short { width: 72%; }
.skeleton-btn { width: 48%; height: 2.5rem; }
@keyframes shimmer {
